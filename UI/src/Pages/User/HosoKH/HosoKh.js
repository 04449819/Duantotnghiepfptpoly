...
const HosoKh = () => {
  ...
  const fetchCustomerData = async () => {
    try {
      const response = await axios.get(
        `https://localhost:7095/api/KhachHang/GetById?id=${userId}`
      );
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(
        `https://localhost:7095/api/DiaChiKhachHang/getalldiachikh?id=${userId}`
      );
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        `https://localhost:7095/api/HoaDon/khachhana/${userId}`
      );
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 9:
        return (
          <FaExclamationCircle color="red" title="Chờ xác nhận hoàn hàng" />
        );
      default:
  ...
  const renderTrangThaiGiaoHang = (trangThai) => {
    ...
    return (
      <div className="d-flex align-items-center">
        ...
        <span className="ms-2">
          {trangThaiGiaoHangDict[trangThai] || "Không xác định"}
        </span>
      </div>
    );
  };
            ...
            <MDBCard className="mb-4">
              <MDBCardBody className="text-center">
                <MDBCardImage
                  ...
                  fluid
                />
                <p className="text-muted mb-1 info-value">
                  {customer?.ten || "Chức vụ"}
                </p>
                ...
                <MDBBtn onClick={handleEditCustomer} className="btn">
          ...
          <MDBCol lg="8">
            <MDBCard className="mb-4">
              <MDBCardBody className="customer-info">
                ...
                <MDBRow>
                  <MDBCol sm="3">
                    <MDBCardText className="info-label">
                      Số Điện Thoại
                    </MDBCardText>
                  </MDBCol>
                  <MDBCol sm="9">
                          ...
                          <MDBListGroupItem key={order.id}>
                            <MDBCardText>
                              ...
                              <div className="order-details">
                                ...
                                {shouldShowNgayNhanHang(
                                  order.trangThaiGiaoHang
                                ) && (
                                  <p>
                                    <strong>Ngày nhận hàng:</strong>{" "}
                                    {formatDate(order.ngayNhanHang)}
                                  </p>
                                )}
                                <p>
