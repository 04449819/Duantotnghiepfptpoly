    ...
    setTimeout(async () => {
      try {
        const res = await axios.post(
          `https://localhost:7095/api/KichCo/ThemKichCo?ten=${ten}&trangthai=${trangthai}`
        );
    ...
    setTimeout(async () => {
      try {
        const res = await axios.put(
          `https://localhost:7095/api/KichCo/${id}?ten=${ten}&trangthai=${trangthai}`
        );
        console.log(res.data);
        if (res.data !== null) {
          ...
          toast.error("Sửa kích thước thất bại");
        }
        dispath(SetLoading(false));
      } catch (error) {
        toast.error("Sửa kích thước thất bại");
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>
            {props.item === undefined ? "Thêm kích thước" : "Sửa kích thước"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form noValidate validated={validated} onSubmit={handleSubmit}>
            <Form.Group as={Col} md="6" controlId="validationCustom03">
              ...
              <Form.Control.Feedback type="invalid">
                Tên kích thước không được bỏ trống.
