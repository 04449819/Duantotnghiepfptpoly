...
const QuanLyNhanVienPage = () => {
 ...
 const laydata = async(SoTrang)=>{
  try {
    let res = await axios.get(`https://localhost:7095/api/NhanVien/GetAll?pageIndex=${SoTrang}&pageSize=10`);
    if (SoTrang ==1){
  ...
  const handleClickEdit = async (idNhanVien) =>{
    const res = await axios.get(`https://localhost:7095/api/NhanVien/GetById?Id=${idNhanVien}`)
    if (res.status == 200){
          ...
          <tbody>
            {data.map((item,index) =>{
               ...
               return (
                ...
                <td>Nhanvien</td>
                { <td><button onClick={()=> handleClickEdit(item.id)} className="edit">Edit</button></td>}
                </tr>
               )
            })}
          </tbody>
