...
const QuanLyDanhGia = () => {
  ...
  const fetchDanhGia = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7095/api/DanhGia/GetAllDanhGiaView"
      );
  ...
  const handleSubmitPhanHoi = async (e) => {
    ...
    try {
      await axios.put(
        `https://localhost:7095/api/DanhGia/GuiPhanHoi/${danhGiaSelected.id}?phanHoi=${phanHoi}`
      );
      await fetchDanhGia();
      handleCloseModal();
    } catch (error) {
  ...
  const filteredDanhGia = danhSachDanhGia.filter((item) => {
    ...
    if (filterOption === "responded")
      return item.phanHoi && item.phanHoi.trim() !== "";
    if (filterOption === "notResponded")
      return !item.phanHoi || item.phanHoi.trim() === "";
    return true;
  });
    ...
    <div className="container mt-4">
      ...
      <Dropdown className="mb-3">
        ...
        <Dropdown.Menu>
          ...
          <Dropdown.Item onClick={() => setFilterOption("responded")}>
            Đánh giá đã phản hồi
          </Dropdown.Item>
          <Dropdown.Item onClick={() => setFilterOption("notResponded")}>
        ...
        <tbody>
          {filteredDanhGia.map((item) => (
            <tr key={item.id}>
              <td>{item.tenKH}</td>
              <td>
                {item.tenSanPham + " - " + item.kichCo + " - " + item.mauSac}
              </td>
              <td>{item.binhLuan}</td>
              <td>
                {[...Array(5)].map((_, index) => (
                  <span
                    key={index}
                    style={{ color: index < item.sao ? "gold" : "gray" }}
                  >
                    ★
                  </span>
                ))}
          ...
          {danhGiaSelected && (
            <Form onSubmit={handleSubmitPhanHoi}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  value={danhGiaSelected.binhLuan}
                  readOnly
                />
              </Form.Group>
              <Form.Group className="mb-3">
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  type="text"
                  value={danhGiaSelected.ngayDanhGia}
                  readOnly
                />
              </Form.Group>
              <Form.Group className="mb-3">
