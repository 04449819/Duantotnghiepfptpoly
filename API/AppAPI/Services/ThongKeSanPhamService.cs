...
namespace AppAPI.Services
{
    public class ThongKeSanPhamService : IThongKeSanPhamService
    {
        ...
        {
			var top = await context.ChiTietHoaDons.Include(a => a.ChiTietSanPham).Include(a => a.ChiTietSanPham.SanPham).Where(a => a.HoaDon.NgayTao.Year == year).GroupBy(a => a.ChiTietSanPham.SanPham).OrderByDescending(a => a.Sum(s => s.SoLuong)).Select(a => a.Key).Take(10).ToListAsync(); return top;
